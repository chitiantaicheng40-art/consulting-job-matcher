    ...
    for (const url of urls) {
      try {
        ...
        if (isPwcManualUrl) {
          ...
          try {
            console.log("===== Accenture manual extract =====");
            const accentureRenderedText = await fetchAccentureRenderedText(url);
            console.log("Accenture rendered text length:", accentureRenderedText.length);
            console.log("Accenture rendered text sample:", String(accentureRenderedText || "").slice(0, 5000));
            console.log("Accenture includes 応募:", String(accentureRenderedText || "").includes("応募"));
            console.log("Accenture includes 必須:", String(accentureRenderedText || "").includes("必須"));
            console.log("Accenture includes 募集要項:", String(accentureRenderedText || "").includes("募集要項"));
            console.log("Accenture includes Qualifications:", String(accentureRenderedText || "").includes("Qualifications"));

            const accentureJob = extractAccentureManualJobFromText(
              accentureRenderedText || pageData.text,
              url
            );

            console.log(accentureJob.title);
            console.log(accentureJob.mustRequirements);
            console.log("====================================");
            normalizedJobs = [accentureJob];
          } catch (e) {
            console.error("Accenture manual extract error:", url, e.message);
            errors.push({
              url,
              message: `Accenture求人抽出に失敗しました: ${e.message}`
            });
            continue;
          }
